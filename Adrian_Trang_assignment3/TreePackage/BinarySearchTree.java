                ...
                implements SearchTreeInterface<T>
{
        ...
        while (!found)
        {
            ...
            else if (comparison < 0)
            {
                ...
                else
                {
                    ...
                    BinaryNode<T> pred = findPredecessorByParent(currentNode);
                    if (pred != null) {
                        pred.setThread(newNode);
                    }
                } // end if
            }
            else
    ...
    */
    private BinaryNode<T> findPredecessorByParent(BinaryNode<T> node) {
        BinaryNode<T> current = node;
        while (current.hasParent()) {
            BinaryNode<T> p = current.getParent();
            if (p.getRightChild() ==current) {
                return p;
            }
            current = p;
        }
    ...
    private BinaryNode<T> findInorderPredecessor(BinaryNode<T> node) {
        if (node.hasLeftChild()) {
            BinaryNode<T> current = node.getLeftChild();
            while (current.hasRightChild()) {
                current = current.getRightChild();
            }
            return current;
        } else {
            return findPredecessorByParent(node);
        }
    }
                            ...
                            BinaryNode<T> parentNode)
    {
        ...
        BinaryNode<T> pred = findInorderPredecessor(nodeToRemove);
        if (pred != null) {
